...
export type TibiaMarkerAnnotation = {
  type: "marker";
  floor: number;
  color: string;
  point: Point;
...
function getDefaultMarkersPath() {
  const localAppData = process.env.LOCALAPPDATA;
  if (!localAppData) {
    throw new Error("LOCALAPPDATA nao esta definido neste sistema");
  }

  return path.join(localAppData, "Tibia", "packages", "Tibia", "minimap", "minimapmarkers.bin");
}
...
function parseMarker(reader: BinaryReader, end: number): TibiaMarker {
  ...
  while (reader.position < end) {
    ...
    if (wireType === 2) {
      ...
      if (field === 3) {
        marker.text = bytes.toString("utf8").trim();
      }
      continue;
    }
...
export class TibiaMinimapMarkerService {
  static getDefaultMarkersPath() {
    return getDefaultMarkersPath();
  }

  static importDefaultMarkers() {
    const sourcePath = getDefaultMarkersPath();
    if (!fs.existsSync(sourcePath)) {
      throw new Error(`Arquivo nao encontrado: ${sourcePath}`);
    }

    return this.parseMarkersFile(sourcePath);
  }

  static parseMarkersFile(sourcePath: string) {
    const reader = new BinaryReader(fs.readFileSync(sourcePath));
    const annotationsByFloor: Record<number, TibiaMarkerAnnotation[]> = {};
    ...
    while (reader.position < reader.length) {
      ...
      const length = reader.readVarint();
      const marker = parseMarker(reader, reader.position + length);
      const annotation: TibiaMarkerAnnotation = {
        type: "marker",
        floor: marker.z,
    ...
    return {
      sourcePath,
      annotationsByFloor,
      importedCount: Object.values(annotationsByFloor).reduce((total, annotations) => total + annotations.length, 0),
    };
  }
}
