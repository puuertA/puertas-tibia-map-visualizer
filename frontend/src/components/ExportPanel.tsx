...
export function ExportPanel({ floor = 7, annotations = [] }: ExportPanelProps) {
  const [exporting, setExporting] = useState(false);

  async function downloadPng(endpoint: string, body: unknown, filename: string, errorLabel: string) {
    try {
      setExporting(true);
      const response = await fetch(`http://localhost:3333/api/export/${endpoint}`, {
        method: "POST",
      ...
      if (response.ok) {
        ...
        alert(`Nao foi possivel exportar ${errorLabel}`);
      }
    } catch (error) {
      console.error(`Erro ao exportar ${errorLabel}:`, error);
      alert("Erro na exportacao");
    } finally {
      setExporting(false);
    }
  }
